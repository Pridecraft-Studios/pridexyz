...
class ModrinthAPI:
    ...
    def _update_ratelimit(self, response: requests.Response):
        with self._ratelimit_lock:
            try:
                limit = int(
                    response.headers.get("X-Ratelimit-Limit", self._ratelimit_limit)
                )
                remaining = int(
                    response.headers.get(
                        "X-Ratelimit-Remaining", self._ratelimit_remaining
                    )
                )
            ...
            self._log_debug(
                f"[RL] Updated rate limit: limit={limit}, remaining={remaining}, reset={reset}"
            )
    ...
    def _respect_ratelimit(self):
        self._log_debug(
            f"[RL] Checking rate limit: remaining={self._ratelimit_remaining}, "
    ...
    def _request(
        self, method: str, endpoint: str, api_version: int = 2, **kwargs
    ) -> Any:
        ...
        try:
            ...
            if response is not None:
                self.logger.error(
                    f"[REQ] HTTP error {response.status_code}: {response.text[:500]}"
                )
            else:
    ...
    def _open_files(paths: List[Path]) -> Iterator[Dict[str, IO[bytes]]]:
        with ExitStack() as stack:
            files = {
                f"file{idx}": stack.enter_context(p.open("rb"))
                for idx, p in enumerate(paths)
            }
            yield files

    def create_project(
        self, project: NewProject, icon_path: Optional[Path] = None
    ) -> DictKV:
    ...
    def create_version(
        ...
        with ExitStack() as stack:
            ...
            return self._request(
                "POST", "/version", data={"data": json.dumps(payload)}, files=files
            )
    ...
    def add_gallery_image(self, id_or_slug: str, image: GalleryImage) -> None:
        ...
        with image.image_path.open("rb") as img_file:
            self._request(
                "POST", f"/project/{id_or_slug}/gallery", params=params, data=img_file
            )

    def delete_gallery_image(self, id_or_slug: str, image_url: str) -> None:
        self._request(
            "DELETE", f"/project/{id_or_slug}/gallery", params={"url": image_url}
        )
    ...
    def parallel_requests(
        cls, requests_list: List[Callable[[], Any]], max_parallel: int = 6
    ) -> List[Any]:
        ...
        with ThreadPoolExecutor(max_workers=max_parallel) as executor:
            future_to_index = {
                executor.submit(wrapper, fn): i for i, fn in enumerate(requests_list)
            }
